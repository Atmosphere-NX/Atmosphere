...
struct sdram_param {
    ...
    uint8_t src_high_bit:5;
    uint8_t is_constant:1;
    uint8_t src_low_bit;

    uint8_t dst_high_bit;
    uint8_t dst_low_bit;
};

static inline void sdram_lp0_save_one_param(struct sdram_params *sdram, volatile tegra_pmc_t *pmc, const struct sdram_param *param) {
    uint32_t mask = 0xffffffff >> (31 - (param->src_high_bit - param->src_low_bit));
    uint32_t value = 0;

    if(param->is_constant) {
        value = param->src_offset;
    } else {
        uint32_t *src = ((uint32_t*) sdram) + param->src_offset;
        value = ((*src >> param->src_low_bit) & mask);
    }

    uint32_t *dst = ((uint32_t*) pmc) + param->dst_offset;
    *dst &= ~(mask << param->dst_low_bit);
    *dst |= value << param->dst_low_bit;
}

static void sdram_lp0_save_parameter_array(struct sdram_params *sdram, volatile tegra_pmc_t *pmc, const struct sdram_param *parameter_array, size_t count) {
    for(size_t i = 0; i < count; i++) {
        sdram_lp0_save_one_param(sdram, pmc, &parameter_array[i]);
    }
}
...
#define s(param, src_bits, pmcreg, dst_bits)    \
    { \
        .src_offset = offsetof(struct sdram_params, param) / sizeof(uint32_t), \
        .dst_offset = offsetof(tegra_pmc_t, pmcreg) / sizeof(uint32_t), \
        .is_constant = 0, \
        .src_high_bit = (1 ? src_bits), \
        .src_low_bit = (0 ? src_bits), \
        .dst_high_bit = (1 ? dst_bits), \
        .dst_low_bit = (0 ? dst_bits) \
    }

#define c(value, pmcreg, dst_bits)  \
    { \
        .src_offset = value, \
        .dst_offset = offsetof(tegra_pmc_t, pmcreg), \
        .is_constant = 1, \
        .src_high_bit = (1 ? dst_bits), \
        .src_low_bit = (0 ? dst_bits), \
        .dst_high_bit = (1 ? dst_bits), \
        .dst_low_bit = (0 ? dst_bits) \
    }

#define s32(param, pmcreg) s(param, 31:0, pmcreg, 31:0)
#define c32(value, pmcreg) c(value, pmcreg, 31:0)
    ...
    static const struct sdram_param parameters_0[] = {
    ...
    sdram_lp0_save_parameter_array(sdram, pmc, parameters_0, sizeof(parameters_0)/sizeof(struct sdram_param));

    switch (sdram->MemoryType)
    {
    case NvBootMemoryType_LpDdr2:
    case NvBootMemoryType_LpDdr4: {
        static const struct sdram_param parameters_1[] = {
        ...
        sdram_lp0_save_parameter_array(sdram, pmc, parameters_1, sizeof(parameters_1)/sizeof(struct sdram_param));
    ...
    static const struct sdram_param parameters_2[] = {
    ...
    sdram_lp0_save_parameter_array(sdram, pmc, parameters_2, sizeof(parameters_2)/sizeof(struct sdram_param));
}
